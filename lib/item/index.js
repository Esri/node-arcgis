...
export default function Organization (arcgis) {
  ...
  var getUsername = function () {
    return arcgis.request({url: `portals/self`})
  ...
  return function getItem (itemId, cb) {
    var promise = get(itemId).then(function (i) {
      ...
      i.changeOwner = function (username, cb) {
        ...
        var changeOwnerPromise = getUsername()
        .then(function (username) {
          return arcgis.request({
      ...
      i.deleteProtected = function (boolean, cb) {
        var protectedPromise = getUsername()
        .then(function (username) {
          if (boolean == true) {
            return arcgis.request({
              url: `content/users/${username}/items/${itemId}/protect`,
              post: true
            })
            .then(function (results) {
              return i.get(itemId)
            ...
            return arcgis.request({
              url: `content/users/${username}/items/${itemId}/unprotect`,
              post: true
            })
            .then(function (results) {
              return i.get(itemId)
      ...
      i.delete = function (cb) {
        var deletePromise = getUsername()
        .then(function (username) {
          return arcgis.request({
            url: `content/users/${username}/items/${itemId}/delete`,
            post: true
          })
        })
